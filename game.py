...
class SnakeGame:
    def __init__(self, hor=60, ver=40, tilesize=10, fps=50, visual=False, obstacles=15, mapa=None):
    ...
    def printstatus(self):
        ...
        player = [p for p in players if p.name == 'student']
    ...
    def writeFile(self):
       ...
       if self.fps == 20:
           f = open("global_file_20.txt",'a')
       elif self.fps == 30:
           f = open("global_file_30.txt",'a')
       elif self.fps == 40:
           f = open("global_file_40.txt",'a')
       elif self.fps == 50:
           f = open("global_file_50.txt",'a')
       for p in self.players:
           media_tempos = sum(p.agent.times)/len(p.agent.times)
           f.write("{},{},{},{},{},{},{},{},{},{}\n".format(self.fps,p.name,p.agent.food_eaten,p.agent.food_lost,p.agent.points_gained,p.agent.lost_points,p.agent.crashed,p.agent.winner,media_tempos,self.mapa))
       f.close()
    ...
    def update(self,snake):
        ...
        if (abs(snake.agent.direction[0]) + abs(snake.agent.direction[1])) > 1:
            ...
            if snake.agent.name == 'student':
                f = open("deads.txt",'a')
                f.write("{} : {} tried to teleport or move diagonaly -> DEAD\n".format(datetime.datetime.now().time(),snake.agent.name))
                f.close()
            ...
            if head in alive.body:
                if head == alive.body[0]: #in case of head to head collision, kill both of the snakes
                    if snake.agent.name != alive.agent.name:
                        ...
                        winner = None
                        for p in self.players:
                            if p.agent.points > p_points:
                                p_points = p.agent.points
                                winner = p
                        if winner != None:
                            logging.debug("Winner : {} with {} points".format(winner.agent.name,winner.agent.points))
                            winner.winner = 1
                        else:
                            logging.debug("Draw")

                        snake.agent.crashed = 1
                        if snake.agent.name == 'student':
                            f = open("deads.txt",'a')
                            f.write("{} : {} crashed against {} -> BOTH DEAD\n".format(datetime.datetime.now().time(),snake.agent.name, alive.agent.name))
                            f.close()
                        self.gameKill(alive)
                    else:
                        ...
                        if snake.agent.name == 'student':
                            f = open("deads.txt",'a')
                            f.write("{} : {} crashed against itself -> Suicide...\n".format(datetime.datetime.now().time(),snake.agent.name))
                            f.close()
                self.gameKill(snake)
                return AgentUpdate.died
        if head in self.obstacles:#hit an obstacle
            ...
            if snake.agent.name == 'student':
                f = open("deads.txt",'a')
                f.write("{} : {} crashed against obstacle -> DEAD\n".format(datetime.datetime.now().time(),snake.agent.name))
                f.close()
            return AgentUpdate.died
            ...
            snake.point(10)
            snake.agent.food_eaten += 1
            snake.agent.points_gained += 10
            other = [p for p in self.players if p.name != snake.name ]
            other[0].agent.food_lost += 1
        ...
        while len([p for p in self.players if not p.IsDead]) > 1:
            ...
            for player in [a for a in self.players if not a.IsDead]:
                ...
                if player.name == 'student':
                    ...
                    player.agent.updateDirection(maze) #update game logic (only for alive players)
                new_s = pygame.time.get_ticks()
                f = pygame.time.get_ticks() - player.latency
                d = (new_s-s)
                player.agent.times.append(d)
                #input("CLOCK")
                ...
                if f-s > 1000*(1/self.fps)/2:
                    ...
                    #if player.name == 'student':
                    #    input("time")
                    player.agent.lost_points = player.agent.lost_points - 10
                    player.point(-10)   #we penalize players that take longer then a half a tick
                if player.name == 'student':
                    m = player.agent.field.field
                    if self.screen != None:
                        for l in range(0,self.hortiles):
                            for c in range(0,self.verttiles):
                                if m[c][l] == 0:
                                    pygame.draw.rect(self.screen,pygame.Color("Green"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                if m[c][l] == -1:
                                    pygame.draw.rect(self.screen,pygame.Color("Yellow"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                #if m[c][l] == -4:
                                #    pygame.draw.rect(self.screen,pygame.Color("Brown"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                if m[c][l] == -2:
                                    pygame.draw.rect(self.screen,pygame.Color("Orange"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                if m[c][l] == -3:
                                    pygame.draw.rect(self.screen,pygame.Color("Black"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                #if m[c][l] == 2:
                                #    pygame.draw.rect(self.screen,pygame.Color("Purple"),(l*self.tilesize,c*self.tilesize,self.tilesize,self.tilesize),0)
                                pass
                    if self.screen != None:
                        lm = player.agent.landmarks_pos
                        for l in lm:
                            pygame.draw.rect(self.screen,pygame.Color("Pink"),(l[0]*self.tilesize,l[1]*self.tilesize,self.tilesize,self.tilesize),0)
            ...
            if self.screen != None:
                for player in self.players: #print players
                    if player.name == 'student':
                        #m = numpy.ndarray(shape = (self.verttiles,self.hortiles))
                        #for f in player.agent.getOpenNodes():
                        #    pygame.draw.rect(self.screen,pygame.Color(255, 255, 255, 127),(f.state[0]*self.tilesize,f.state[1]*self.tilesize,self.tilesize,self.tilesize),0)
                    #    for p in player.agent.path:
                    #        pygame.draw.rect(self.screen,pygame.Color(0, 255, 255, 127),(p[0]*self.tilesize,p[1]*self.tilesize,self.tilesize,self.tilesize),0)
                        #for f in player.agent.getExplored():
                        #    pygame.draw.rect(self.screen,pygame.Color("Grey"),(f[0]*self.tilesize,f[1]*self.tilesize,self.tilesize,self.tilesize),0)
                        #for f in player.agent.getExploredNodes():
                            #Color = Grey
                        #    pygame.draw.rect(self.screen,pygame.Color("Grey"),(f[0]*self.tilesize,f[1]*self.tilesize,self.tilesize,self.tilesize),0)
                        #m[f.state[1]][f.state[0]] = f.heuristic
                        #m[player.body[0][0]][player.body[0][1]] = -10
                        #numpy.savetxt("matrix",m,fmt='%.2f',delimiter='  ')
                        #input()
                        #if len(player.agent.getExplored())/len(player.agent.path) > 3:
                        #    input("Check")
                    #head + rest of body
                        pass
