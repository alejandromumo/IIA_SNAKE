...
def main(argv):
    ...
    fps = 10
    try:
        opts, args = getopt.getopt(argv,"hm:s:o:p",["help","map=","disable-video","student-agent=","oponent-agent=","proxy","fps","quarenta","trinta","vinte","cinquenta"])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            network = True
        elif opt in("-f", "--cinquenta"):
            fps = 50
        elif opt in("--quarenta"):
            fps = 40
        elif opt in("--trinta"):
            fps = 30
        elif opt in("--vinte"):
    ...
    if network:
        ...
        try:
            snake=SnakeGame(hor=60, ver=40, fps= fps, visual=visual, obstacles=15, mapa=inputfile)
            snake.setPlayers([  
